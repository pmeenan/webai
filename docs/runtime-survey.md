...
| **Chrome Prompt API** | Text, image, and audio input; text output. Audio requires a GPU. | JSON Schema and regular-expression constraints through `responseConstraint`. Stable web pages expose no sampling controls; the Chrome 151+ `samplingMode` work remains an origin-trial experiment. Capability probing must use the same options as session creation (re-verified 2026-07-19, D-030). | Browser API, so no library dependency license; actively shipped by Chrome. | **Confirmed**: implemented locally as the second runtime in M4; live supported-profile exit check remains. |
