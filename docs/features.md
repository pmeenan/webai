...
| MTP/speculative-decoding capability and acceleration testing | confirmed | M3 proves the pinned wllama browser path or records its exact gap; M7 maintains a versioned matrix across all adapters; M8 adds controlled target-only vs. MTP benchmarks. Upstream-engine support, a downloaded companion, and actual browser acceleration are separate claims (D-026). |
