...
| Chrome built-in Prompt API, including model download flow | confirmed | M4 implemented locally (D-030). Browser-managed Gemini Nano only — a separate acquisition path from HF downloads; window-only, not available in workers (D-007); available to web pages in stable Chrome from 148. Stable web exposes availability, normalized download progress, stateful streaming, context usage/window and overflow, abort, and destroy, but no sampling controls or backend identity; sampling remains experimental/origin-trial-only (re-verified 2026-07-19). Availability remains a volatile probe, never a browser-version inference. |
...
| Stop/abort generation, regenerate, edit-and-resend | confirmed | M3 exposes stop/abort for the active generation. Prompt API preserves its browser session; wllama 3.5.1 has no bounded native per-completion interrupt, so emergency Stop terminates the dedicated worker, preserves partial output, and requires an explicit model reload (D-010, D-028, D-039, RE-028). Regenerate and edit-and-resend land in M6. |
